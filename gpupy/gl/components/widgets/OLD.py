...
class CartesianGrid(Widget):
   ...
   #     um            = self.program.uniform_manager
   #     um.size       = self.size 
   #     um.position   = self.position 
   #     um.w_grid     = self.grid_width 
   #     um.d_grid     = self.grid 
   #     um.c_bg       = self.background_color 
   #     um.c_line     = self.line_color 
   #     um.resolution = self.resolution 
   #     um.camera     = GlConfig.STATE.RESERVED_BUFFER_BASE['gpupy.gl.camera']
